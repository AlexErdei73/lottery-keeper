...
## What has been addressed from the above
    ...
    * 4. I have written automated tests for the modules in the gamelogic folder to make sure that everything is working as it is supposed to do. I used Vitest, because Jest does not
    integrate with Vite well. The syntax is almost identical with
    Jest.

    * 5. I have added a short text description for the opening page, so the user knows what the app actually is. I also made
    the animation movments cancelable by user setup. This might be important for certain users with accessibilty needs.

## Is there any point in the specification, which can be interpretted more ways?

    In my opinion, the answer is yes. The following part about players is not straightforward:

    "- Can check how many correct guesses they had and the corresponding payout after the drawing."
    ...
    In my interpretation the sort button click rearranges the buttons according to the number of correct guesses. The
    ...
    for all the games in the operator page. It can also be done here too.

    At the moment I will keep it as it is, because players cannot have too many individual games because of their starting 10000 credit. They can check their individual
    winning tickets easily and they also see how many non-winning tickets they have got.

## Problems with local storage

    Local storage may not be enough to store more than 40-50000 tickets in the current setup. The number of simulations are
    maxed in 10000, which does not cause any problem. If you keep
    pressing the Simulation button you get an error message at some point. The app won't crash but it warns you in a friendly manner that you run out of local storage, so you need to reset the app, because it cannot hold more data.

    What are our options to scale it better?
    1. We can redesign a bit the information, which stored. We can only store the important bits and calculate the rest programatically. This way we gain on the storage, but lose on the computation time. Games could be stored in two arrays:

    - This would store the player's games, but only the numbers, no other information.

    - Another array can store the numbers of simulation games, but again only the numbers.

    At the moment we store all the rounds until the user resets the app, but it is not a requirement, so we can change it. The rest can be calculated from this programatically for each ticket. This way we may go up to 100000 - 200000 or maybe slightly more.

    2. Browsers has an indexedDB API, which can store objects like the state and fast queries are possible too. The limit of this storage is much higher than the local storage, so it is more than enough for even 1000 0000 0000 games. The bottleneck might be the execution efficiency in this case.

    3. It might be more convenient to store the state online in a database in the cloud. The simplest possibility is a BaaS service like Firebase. This is slightly more complex to set up than the previous things but it has an advantage. It can be used in the same time on remote computers, so the player and the operator does not need to be on the same device. It is simple to add more players and user authentications for them. For a couple of people the app can even run on free
    tier, so no cost involved. The app can be almost the same just it needs another module, which keeps Firebase Firestore connected to the UI state. It has Websockets API, so the updates between different computers are practically instantenious.

    4. We can write a REST API for it, maybe with Node.js in JS. We can host the back-end app with a cloud provider, which is never free and the app can use MongoDB on the Atlas cluster for example. I have several full-stack apps like this. We can also make it work with Websockets with the socket.io library.

    The easiest and cheapest option is using Firebase, writing server side JS code is worth it for a larger user base. In this case the hosting can become more expensive, because the resources need to handle higher amount of simultanous requests. At the moment local storage is the simplest solution, because the specification does not contain how well it should scale.
